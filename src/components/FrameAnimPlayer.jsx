      ...
      <div className={`${cssPrefix}-player native ${className}`} style={{ width: displayWidth, maxWidth: '100%', ...style }}>
        ...
        {!hideInfo && !compact && (
          <div className={`${cssPrefix}-info`}>
            ...
            {pixivUrl && !hideLink && <a className={`${cssPrefix}-pixiv-link`} href={pixivUrl} target="_blank" onClick={e => e.stopPropagation()}>Pixiv →</a>}
          </div>
        )}
      </div>
    );
    ...
    >
      ...
      {!hideInfo && !compact && (
        <div className={`${cssPrefix}-info`}>
          ...
          {author && <span className={`${cssPrefix}-author`}>@{author}</span>}
          {pixivUrl && !hideLink && (
            <a className={`${cssPrefix}-pixiv-link`} href={pixivUrl} target="_blank"
